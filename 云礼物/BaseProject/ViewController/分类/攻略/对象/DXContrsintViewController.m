...
@interface DXContrsintViewController ()
@property (strong,nonatomic) CategoriesViewController *vc;
@end

@implementation DXContrsintViewController
+ (DXContrsintViewController *)standardDXContentNav{
    static DXContrsintViewController *vc = nil;
...
/** 提供题目数组 */
+ (NSArray *)itemNames{
    return @[@"女盆友",@"男票",@"小盆友",@"家人",@"基友",@"闺蜜",@"同事"];
}
/** 提供每个题目对应的控制器的类型。题目和类型数量必须一致 */
+ (NSArray *)viewControllerClasses{
    NSMutableArray *arr = [NSMutableArray new];
    for (id obj in [self itemNames]) {
        [arr addObject:[DXContentViewController class]];
    }
    return [arr copy];
}
