...
@interface FGContrsintViewController ()
@property (strong,nonatomic) CategoriesViewController *vc;
@end

@implementation FGContrsintViewController
+ (FGContrsintViewController *)standardFGContentNav{
    static FGContrsintViewController *vc = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        vc = [[FGContrsintViewController alloc]initWithViewControllerClasses:[self viewControllerClasses] andTheirTitles:[self itemNames]];
        vc.keys = [self vcKeys];
...
/** 提供题目数组 */
+ (NSArray *)itemNames{
    return @[@"创意",@"萌萌哒",@"小清新",@"科技范",@"奇葩",@"设计感"];
}
/** 提供每个题目对应的控制器的类型。题目和类型数量必须一致 */
+ (NSArray *)viewControllerClasses{
    NSMutableArray *arr = [NSMutableArray new];
    for (id obj in [self itemNames]) {
        [arr addObject:[FGContentViewController class]];
    }
    return [arr copy];
}
